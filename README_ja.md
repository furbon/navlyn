...
2. **必要な読むキューだけを作る。** 小さい事実で足りないと分かったときだけ、`impact`、`related`、`context-pack --goal modify` へ進みます。
3. **編集後に実際の diff を確認する。** `changed-symbols` または `review-diff --profile evidence` を使い、変更されたシンボルが編集前の anchor と合っているか確認します。
...
| 編集後の diff を確認する | `changed-symbols --profile compact` | 変更シンボルが編集前の anchor と一致する、またはズレの理由が説明できるとき。 |

...
**Navlyn は、C#/.NET のコーディングエージェントが編集前に読む Roslyn 由来の意味論的証拠レイヤーです。**
...
C# エージェントにコードを触らせる前の **preflight** として使ってください。まず正確なシンボルを固定し、必要なソースと関係だけを確認し、Navlyn の外で編集した後に dirty diff を確認します。
...
最初の導線は [`docs/navlyn-first-10-minutes.md`](docs/navlyn-first-10-minutes.md) にまとめています。

## 3 つのエージェント workflow

1. **編集前に正しいシンボルを固定する。** `resolve-target` を使い、返った `candidateId` を `symbol-source`、`references`、`about` で再利用します。
2. **編集前証拠を 1 つの envelope にまとめる。** `edit-preflight` で anchor、上限付き source/context、関連テスト、confidence、次に走らせる guard を取得します。
3. **編集後に実際の diff を確認する。** `post-edit-guard` または `wrong-symbol-guard` を使い、必要なときだけ `review-diff --profile evidence` へ広げます。
...
| エージェント編集を準備する | `edit-preflight --candidate-id sym:v1:... --goal modify` | anchor、上限付き証拠、既知の不明点、post-edit guard command が揃ったとき。 |
...
- **エージェント guardrail**: wrong-symbol 回避のための `edit-preflight`、`post-edit-guard`、`wrong-symbol-guard`、change intent、handoff、confidence ledger。
...
実際の PR/diff review と post-edit guard には `--tool-profile review`、`navlyn_edit_preflight` と intent/handoff/confidence pack には `edit`、`navlyn_batch` を含む完全な互換 surface が必要な場合だけ `full` を使います。
...
1 つ目は曖昧な名前を C# シンボルへ固定します。2 つ目はその意図した対象の edit preflight envelope を作ります。3 つ目は明示した Git 範囲の review envelope を確認します。PR の ref に置き換えるか、dirty branch では ref 指定を省略できます。

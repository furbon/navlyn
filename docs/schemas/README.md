...
- `navlyn-resolve-target-result.schema.json`: high-use `resolve-target` target-selection result shape, including optional `ambiguitySummary` fields for fail-closed candidate handling.
...
The schemas intentionally focus on automation-critical envelopes and high-risk command results rather than every command-specific domain object. They cover target selection, file-first source reading, workspace freshness, workflow/MCP envelopes, edit preflight, guardrails, and scoped reverse-edge metadata. They do not attempt to schema every application-domain section, review-pack finding subtype, DI fact, route fact, EF fact, or nested Roslyn symbol detail; those facts are documented in `docs/navlyn-cli-commands.md` and guarded by focused resolver tests, CLI contract tests, and representative golden snapshots.

When a command adds automation-critical top-level fields, ambiguity/freshness/cost metadata, or guardrail risk fields, add or update a focused schema here and cover it from `navlyn.Tests/Contracts`.

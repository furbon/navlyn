...
- Use command `navlyn-mcp` with args `["--workspace", "<workspace>"]`.
...
- Use `navlyn_edit_preflight` and guard tools only for concrete edit evidence.
- Use `navlyn_review_diff` only for actual diff review facts.

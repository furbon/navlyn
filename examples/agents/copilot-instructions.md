...
- In MCP, the default `reader` profile exposes file-first and selected-symbol tools. Use `navlyn_file_outline` for one known C# file and `navlyn_symbol_source` or `navlyn_symbol_edges` for one selected symbol fact.

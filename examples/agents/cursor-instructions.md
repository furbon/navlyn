...
- For MCP clients, default `reader` profile exposes file-first and selected-symbol tools. Use `navlyn_file_outline` for one known C# or Visual Basic file, `navlyn_resolve_target` for symbol intent, then `navlyn_symbol_source` or `navlyn_symbol_edges` for one precise fact.
